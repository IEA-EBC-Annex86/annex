...
raw_df$X <- as.POSIXct(raw_df$X, format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
class(raw_df$X)
```
...
```{r}
annex_df <- annex(Light + Pressure + Radon_AVG + CO2 + T + rH + VOC ~ datetime | study + home + room, data = prepared_df)
head(annex_df)
```
...
plot(annex_df)
plot(annex_df, start = "2011-03-01")
plot(annex_df, start = "2011-03-01", end = as.POSIXct("2011-03-10"))
```
