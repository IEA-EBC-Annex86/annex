...
Some of the lines in the file are commented (start with `#`) which we
would like to ignore on import:

```{r}
config <- read.csv("demo_UIBK_config.csv", comment.char = "#")
head(config)
```
...
To be able to write the statistics into the final XLSX file,
the statistics must be in the wide format (TODO(R): Convert on the fly?).
...
```{r}
stats_wide <- annex_stats(annex_df, format = "wide")
annex_write(stats_wide, file = "final_UIBK.xlsx", user = 123, quiet = TRUE)
```
