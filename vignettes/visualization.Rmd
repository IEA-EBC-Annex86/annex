...
# Unique ID as an interaction of study, home, room, season, and tod
tmp$ID <- with(tmp, interaction(study, home, room, season, tod))
```
...
```{r viz_ggplot_CO2, fig = TRUE, fig.width = 10, fig.height = 8}
ggplot(subset(tmp, variable == "CO2")) +
    geom_line(aes(x = p, y = value, col = interaction(season, tod, sep = " - "), group = ID)) +
    facet_wrap(~ room) +
