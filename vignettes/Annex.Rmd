...
[`Bedroom.txt`](../inst/data/demo_Bedroom.txt) (shipped
with the package).
...
which can easily be read using _Rs_ `read.table()`
...
The first row (with datetime information) is called `"X"` as the
column does not have a name defined in the text file. Can be
renamed in different ways, e.g.,:
