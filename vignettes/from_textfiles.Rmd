...
interfacing functions like [utils::read.csv()], [utils::read.delim()] etc.
...
the function [annex::annex_check_config()] can be used. In case problems would
...
calling [annex::annex_prepare()] but can be handy during development.
...
observations belong to. [annex::prepare_annex()] is a helper function
...
convert the column (column `X` in `raw_df`) and call [annex::annex_prepare()] again.
...
[annex::annex_prepare()] performs a series of tasks:

* Checking the `config` object (calls [annex::annex_check_config()] internally).
...
Once the data set is prepared properly (note that [annex::annex_prepare()]
is a convenience function, can also be done manually) the final object
can be prepared.

### Prepare `annex` object

[annex::annex()] is the creator function which creates an object of class
...
Based on the object returned by [annex::annex()] the analysis
can be performed by calling [annex::annex_stats()]. The function
...
[annex::annex_write_stats()] takes up a `annex_stat` object (returned by
[annex::annex_stats()]; long or wide format) and a file name/path where the
